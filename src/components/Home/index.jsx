...
import FeturesPlayList from '../featuresPlayLsts';
import CategoryPlayList from '../categoriesPlayList';
import NewReleases from '../NewReaslesPlayList';
...
const Home = () => {
  ...
  useEffect(() => {
    const fetchPlaylists = async () => {
      ...
      setPlaylists(responseData.playlists.items);
      setLoading(false);
    };
    fetchPlaylists();
  },[]);
  ...
  useEffect(() => {
    const fetchCategoryPlaylists = async () => {
      ...
      const response = await fetch(url);
      const responseData = await response.json();
      setCategoryLists(responseData.categories.items);
      setLoading(false);
  ...
  useEffect(() => {
    const fetchNewReleasesPlaylists = async () => {
      const url = "https://apis2.ccbp.in/spotify-clone/new-releases";
      const response = await fetch(url);
      const responseData = await response.json();
      setnewReleases(responseData.albums.items);
      setLoading(false);
    };
    fetchNewReleasesPlaylists();
  },[]);
  ...
  return (
    ...
    :
    <div className='bg-[#181818] min-h-screen gap-5'>
