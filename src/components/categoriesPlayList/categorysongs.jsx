...
const CategoryDetails = () => {
  ...
  useEffect(() => {
    const fetchSongsDetails = async () => {
      try {
        const url = `https://apis2.ccbp.in/spotify-clone/category-playlists/${CategoryplaylistId}`;
  ...
  return (
    <>
      <Navbar />
      <div className="min-h-screen bg-gradient-to-b from-[#5a2219] to-black text-white pt-20">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 p-6">
          {filteredPlaylists.map((pl) => (
            <Link
              to={`/playlist/${pl.id}`}
              key={pl.id}
              className="bg-[#181818] p-4 rounded-lg hover:bg-[#282828] transition"
            >
